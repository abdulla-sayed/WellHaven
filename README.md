...
A full-stack mental wellness platform that helps people **track their mood, journal their thoughts, and talk to an AI therapist** — backed by a personalized onboarding assessment and rich analytics that surface patterns in their emotional wellbeing over time.
...
- **🤖 AI Therapy Assistant** — A conversational chat interface backed by Azure OpenAI (GPT-3.5 Turbo). Full conversation context is preserved across turns, and every chat session is saved so users can revisit, rename, or delete past conversations.
...
![Onboarding assessment](images/onboarding.png)
...
- **AI**: Azure OpenAI Service (GPT-3.5 Turbo deployment)
- **Validation**: Zod
...
// Format the entire history for the OpenAI API
const formattedMessages = allMessages.map((msg) => ({
  role: msg.isFromUser ? "user" : "assistant",
  content: msg.content,
}));

// Generate the AI response, then persist it alongside the user's message
const aiResponseContent = await generateChatResponse(formattedMessages);
```

The call to Azure OpenAI is centralized in a thin client wrapper:
...
![AI Therapy chat](images/ai-therapy.png)

## Data Model

WellHaven uses Prisma with MongoDB. The schema is organized around a single `User` (keyed by their Clerk auth ID) with related records for journaling, chat, and daily tracking:
...
| **ChatSession** | A named AI therapy conversation belonging to a user                                                            |
...
![Journaling](images/journal.png)

![Mood calendar](images/mood-calendar.png)

## API & Server Actions

### API Routes

- `POST /api/chat` — Send a message; creates/continues a session and returns the AI reply
...
![Insights & analytics](images/summary.png)
...
WellHaven is deployed to **Azure Web App** through a GitHub Actions pipeline that builds the Next.js app on every push to `main` and publishes the artifact to production.

---

<div align="center">

_WellHaven is an evolving project — some features are still under active development._
